...
import { useCycleDetail, useEditCycle } from '../hooks';
import type { CicloUpdateData } from '../models/Ciclo';
import styles from './EditCyclePage.module.css';
...
const EditCyclePage: React.FC = () => {
  ...
  const { ciclo, loading: loadingDetail, error: errorDetail, recargarDatos } = useCycleDetail(cycleId);
  ...
  // Hook para editar ciclo
  const { updateCycle, loading: loadingUpdate } = useEditCycle();
  ...
  useEffect(() => {
    const cargarCiclo = async () => {
      ...
      console.log(`Cargando datos del ciclo con ID: ${cycleId}`);
      setIsLoading(true);
      try {
        await recargarDatos();
      } finally {
    ...
    cargarCiclo();
    // Eliminamos loadingDetail del array de dependencias para evitar loops
  }, [cycleId, recargarDatos, isLoading]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        try {
            ...
            setTimeout(() => {
                navigate(`/admin/ciclo/${cycleId}`);
            }, 1500);
        } catch (apiError: any) {
            // El hook ya re-lanza el error para que lo capturemos aquí
  ...
  if (errorDetail) {
    return (
      <div>
        <h2>Error al cargar el ciclo</h2>
        <p>{errorDetail}</p>
        <button onClick={() => navigate('/admin/ciclos')}>Volver a la lista</button>
      </div>
    );
  }
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.formActions}>
            ...
            <button
              ...
              disabled={submitting || loadingUpdate || !formData.descripcion}
            >
