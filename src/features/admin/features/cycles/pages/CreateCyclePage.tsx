...
import styles from './CreateCyclePage.module.css';
...
const CreateCyclePage: React.FC = () => {
  ...
  // Solo necesitamos la función createCycle y el estado de carga
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Desactivar cualquier notificación anterior
    setNotificacion(prev => ({ ...prev, visible: false }));
    ...
    try {
      ...
      if (success) {
        ...
        if (errorMessage) {
          ...
          // Si no hay un error específico
          mostrarNotificacion('No se pudo crear el ciclo. Por favor, intente nuevamente.', 'error');
        }
      }
    } catch (err: any) {
      console.error('Error al crear ciclo:', err);
      // En caso de que haya un error no capturado por el hook
      mostrarNotificacion(err.message || 'Error inesperado al crear el ciclo', 'error');
    }
  };
