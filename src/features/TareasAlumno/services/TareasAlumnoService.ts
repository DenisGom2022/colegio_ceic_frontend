...
export const TareasAlumnoService = {
       ...
       updateNota: async (
	       ...
	       try {
		       const url = `${API_URL}/tareas-alumno/${id}`;
		       const response = await axios.put<{ message: string; tareaAlumno: any }>(
			       url,
			       { nuevoPunteo },
			       { headers: getAuthHeaders() }
		       );
		       ...
		       console.error('Error al actualizar punteo de tarea de alumno:', error);
		       throw error;
	       }
       }
};
