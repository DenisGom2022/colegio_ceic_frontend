...
import styles from './CrearCatedratico.module.css';
...
import { useCrearCatedratico } from '../../hooks/useCrearCatedratico';
...
const CrearCatedratico = () => {
  const { crearNuevoCatedratico, loading } = useCrearCatedratico();
    ...
    switch (name) {
      case 'dpi':
        return value.trim() === '' ? 'El DPI es requerido' : 
               value.length < 5 ? 'El DPI debe tener al menos 5 caracteres' : '';
      case 'idUsuario':
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      try {
        const response = await crearNuevoCatedratico(formData);
        ...
        if (response.success) {
          ...
          if (response.errorMessage) {
            ...
            setShowNotification(true);
            console.error('Error al crear catedrático:', response.errorMessage);
          } else {
            setErrorMessage("Ocurrió un error al crear el catedrático. Por favor, inténtelo nuevamente.");
            setShowNotification(true);
          }
        }
      } catch (error: any) {
...
export default CrearCatedratico;
