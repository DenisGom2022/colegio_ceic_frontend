...
import styles from './EditarUsuarioNew.module.css';
import { 
  FaUser, 
  FaIdCard, 
  FaPhone, 
...
import type { TipoUsuario } from '../../models/TipoUsuario';

interface FormErrors {
  [key: string]: string;
}

const EditarUsuarioNew = () => {
  const { usuario } = useParams<{ usuario: string }>();
  const navigate = useNavigate();
  const { getUsuario } = useUsuario();
  const { loading: loadingUpdate, error: errorUpdate, updateUser } = useEditarUsuario();
  ...
  useEffect(() => {
    ...
    const cargarDatos = async () => {
      ...
      try {
        ...
        const userData = await getUsuario(usuario);
        if (isMounted && userData) {
          setFormData({
            usuario: userData.usuario,
            primerNombre: userData.primerNombre || '',
            segundoNombre: userData.segundoNombre || '',
            tercerNombre: userData.tercerNombre || '',
            primerApellido: userData.primerApellido || '',
            segundoApellido: userData.segundoApellido || '',
            telefono: userData.telefono || '',
            idTipoUsuario: userData.tipoUsuario?.id || 0
          });
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      try {
        ...
        setTimeout(() => {
          navigate(`/admin/usuario/${formData.usuario}`);
        }, 2000);
        ...
        if (errorUpdate) {
          setNotificationMessage(errorUpdate);
      ...
      <div className={styles.formContainer}>
        ...
        <div className={styles.pageHeader}>
          <div className={styles.breadcrumb}>
            ...
            <Link to={`/admin/usuario/${usuario}`}>{formData.usuario}</Link>
            <span className={styles.breadcrumbSeparator}>/</span>
            <span className={styles.breadcrumbCurrent}>Editar usuario</span>
          </div>
          
          <Link to={`/admin/usuario/${usuario}`} className={styles.buttonCancel}>
            ...
            <div className={styles.formCard}>
              ...
              <div className={styles.formSection}>
                ...
                <div className={styles.formGrid}>
                  ...
                  {/* Se omite el campo correo ya que no está en el modelo */}
                </div>
              </div>
...
export default EditarUsuarioNew;
